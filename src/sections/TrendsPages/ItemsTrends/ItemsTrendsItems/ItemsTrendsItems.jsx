...
const ProjectItems = () => {
  ...
  const processedData = useMemo(() => {
    let filteredData = metaDeckItemStats;

    // Apply category filter
    if (categoryFilter !== "All") {
      filteredData = filteredData.filter((item) => {
        const itemData = items.find((i) => i.key === item.key);
        return itemData?.tags?.includes(categoryFilter.toLowerCase());
      });
    }
    ...
    return filteredData;
  }, [metaDeckItemStats, categoryFilter, searchValue, sortConfig, items]);
  ...
  const renderExpandedContent = useCallback(
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {item?.itemSynergyStats?.slice(0, 3).map((synergy, w) => {
                  const synergyItem = items.find((i) => i.key === synergy);
                  return synergyItem ? (
    ...
    [items, champions, forces, others, mobileFilter]
  );

  // Memoize table rows
  const MemoizedDesktopRow = React.memo(({ item, index }) => {
    const itemData = items.find((i) => i.key === item.key);
    if (!itemData?.key) return null;
      ...
      >
        ...
        <td className={`p-2 ${getCellClass("key")}`}>
          <div className="flex justify-start items-center space-x-1 sm:space-x-2">
            <div data-tooltip-id={`${itemData.key}}`}>
              <ItemDisplay
                item={itemData}
                size="midMedium"
                borderRadius="rounded-[4px]"
                backgroundRadius="rounded-[4px]"
                tooltipId={`${itemData.key}}`}
                showTooltip={true}
              />
            </div>
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center flex-wrap gap-1">
                {itemData.compositions?.map((comp, compIndex) => {
                  const compItem = items.find((i) => i.key === comp);
                  return compItem ? (
  ...
  const MemoizedMobileRow = React.memo(({ item, index }) => {
    const itemData = items.find((i) => i.key === item.key);
    if (!itemData?.key) return null;
            ...
            <table className="w-full min-w-[1100px] relative border-collapse">
              <thead className="sticky top-0 z-50">
                <tr className="bg-[#000000]">
                  ...
                  <th
                    className={`cursor-pointer p-2 font-semibold min-w-[200px] sm:min-w-[220px] md:min-w-[280px] border-b border-[#2D2F37] ${
                  ...
                  <th
                    className={`cursor-pointer p-2 font-semibold min-w-[80px] sm:min-w-[90px] border-b border-[#2D2F37] ${

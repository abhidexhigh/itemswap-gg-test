...
const Set10Tabs = ({ tabs, activeTab, setActiveTab }) => {
  ...
  return (
    <ul className="flex flex-wrap pl-0 mb-4 list-none bg-[#222231] rounded-lg md:!max-w-[90%] md:!mx-auto">
      {tabs?.map((tab, index) => (
        <Link
          key={index}
          href={
          ...
          className={`flex-auto flex-grow-1 p-2 text-center !border-r !border-[#ffffff14] rounded-none font-bold -mb-[1px] text-[16px] cursor-pointer px-[12px] md:p-0 ${
            tab === activeTab
              ? "active text-gray-900 !bg-gray-100 border-r"
              : ""
          } ${activeTab === "Comps" ? "rounded-l-lg" : ""} ${
            activeTab === "Items" ? "rounded-r-lg" : ""
          }`}
